...
class ScheduleController extends Controller
{
    public function index($id, Request $r)
    {
        ...
        $subscription = Subscription::where(['user_id' => Auth::user()->id, 'request_id' => $request->id])->first();
    ...
    public function indexPost($id, Request $r)
    {
        ...
        if (Input::get('iban') !== null) {
            if (!preg_match('/([A-Z]{2})([0-9]{2})([A-Z]{4})([0-9]{8,10})/', Input::get('iban'))) {
                return back()->withErrors(['iban' => "Geen geldige IBAN rekening formaat"]);
            }
        }
        ...
        if (sizeof($mandates) == 0) {
            try {
                $mandate = $mollie->customers->get($customer->id)->createMandate([
                    ...
                    "consumerName" => Auth::user()->firstname . " " . Auth::user()->familyname,
        ...
        return redirect(route('schedule-success', [$request->id, $subscription->id]));
    }
